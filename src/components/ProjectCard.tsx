...
const ProjectCard = ({ project }: ProjectCardProps) => {
    return (
        <Link href={`/projects/${project.slug}`} className="flex group h-full">
            <Card className="bg-card flex flex-col w-full hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-2 transition-all duration-300 relative overflow-hidden card-hover-shimmer rounded-2xl">
                ...
                <div className="p-4 flex flex-col flex-grow">
                    ...
                    <CardContent className="flex-grow p-0 pt-4">
                        <CardDescription className="text-sm text-muted-foreground overflow-hidden whitespace-pre-line" style={{display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical'}}>
                            {project.description}
                        </CardDescription>
                    </CardContent>
